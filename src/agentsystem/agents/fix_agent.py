...
def fix_node(state: DevState) -> DevState:
    ...
    fix_fingerprint = _build_fix_fingerprint(repo_b_path, candidate_files)
    fingerprint_history = list(state.get("fix_fingerprint_history") or [])
    repeated_same_fingerprint = bool(fix_fingerprint) and fingerprint_history.count(fix_fingerprint) >= 2
    repeated_same_review = bool(state.get("review_issue_signature")) and list(state.get("review_issue_history") or []).count(str(state.get("review_issue_signature"))) >= 3
    if attempts > 3 and (repeated_same_fingerprint or repeated_same_review):
        state["fixer_success"] = False
        state["fix_result"] = "Loop detected during reviewer/fixer remediation."
        state["failure_type"] = "loop_detected"
        state["interruption_reason"] = "reviewer_fixer_loop"
        state["error_message"] = "Loop detected after repeated reviewer/fixer attempts with no material progress."
        state["current_step"] = "fix_done"
        state["fix_return_to"] = "__end__"
        return state
    ...
    fingerprint_history.append(fix_fingerprint)
    state["fix_fingerprint_history"] = fingerprint_history
...
def route_after_fix(state: DevState) -> str:
    if state.get("error_message") and str(state.get("fix_return_to") or "") == "__end__":
        return "__end__"
...
def _generate_fix(current_code: str, failure_info: str, target_file: Path | None = None) -> str:
    ...
    if target_file and (
        target_file.suffix.lower() in {".json", ".yaml", ".yml", ".toml", ".ini", ".cfg"}
        or target_file.name.startswith(".env")
        or target_file.name.endswith(".example")
    ):
        return current_code
    if not os.getenv("OPENAI_API_KEY") and (
        "trailing spaces are not allowed" in lowered_failure
        or "tabs are not allowed" in lowered_failure
        or "newline at end of file" in lowered_failure
    ):
...
def _apply_style_hygiene_fix(current_code: str, lowered_failure: str) -> str:
    ...
    if "tabs are not allowed" in lowered_failure:
        expanded = result.replace("\t", "    ")
        changed = changed or expanded != result
        result = expanded

    if "trailing spaces are not allowed" in lowered_failure:
        lines = result.splitlines()
        trimmed = [line.rstrip(" ") for line in lines]
        rebuilt = "\n".join(trimmed)
        if result.endswith("\n") or rebuilt:
            rebuilt = rebuilt + "\n"
        changed = changed or rebuilt != result
        result = rebuilt

    if ("no newline at end of file" in lowered_failure or "newline at end of file" in lowered_failure) and result and not result.endswith("\n"):
        result = result + "\n"
        changed = True

    return result if changed else current_code
...
def _regenerate_agenthire_seed_catalog_artifacts(repo_b_path: Path) -> list[str]:
    data_dir = repo_b_path / "apps" / "api" / "src" / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    script_path = repo_b_path / "scripts" / "seed_agent_marketplace.py"
    script_path.parent.mkdir(parents=True, exist_ok=True)

    taxonomy = {
        "industry": ["Finance", "Sales", "Engineering", "Marketing", "Legal", "Construction", "Support", "HR"],
        "task": ["Research", "Review", "Planning", "Summarization", "Reporting", "Automation"],
        "capability": ["Search", "Analyze", "Generate", "Review", "Monitor", "Coordinate"],
        "integration": ["API", "Slack", "Browser", "MCP", "A2A", "Local"],
        "pricing": ["Free", "Freemium", "Paid", "Enterprise", "Custom Quote"],
    }

    industries = ["Finance", "Sales", "Engineering", "Marketing", "Legal"]
    tasks = ["Research", "Review", "Planning", "Automation"]
    capabilities = ["Search", "Analyze", "Generate", "Monitor"]
    integrations = ["API", "Slack", "Browser", "MCP"]
    pricing_cycle = ["Freemium", "Paid", "Enterprise", "Custom Quote"]
    ...
    for index in range(1, 21):
        ...
        integration = integrations[(index - 1) % len(integrations)]
        pricing = pricing_cycle[(index - 1) % len(pricing_cycle)]
        ...
        agents.append(
            {
                ...
                "tagline": f"{industry} {task.lower()} copilot for teams",
                "description": f"Curated sample Agent {index:02d} for {industry.lower()} teams that need {task.lower()} and {capability.lower()} workflows.",
                "official_url": f"https://example.com/{slug}",
                "demo_url": f"https://example.com/{slug}/demo",
                "pricing_type": pricing,
                "company_name": f"Builder {index:02d}",
                "is_featured": index <= 6,
                "best_for": [industry, task, "Teams"],
                "categories": {
                    "industry": [industry],
                    "task": [task],
                ...
                "evaluation": {
                    "best_for": f"Best for {industry} teams",
                    "setup_difficulty_label": "Easy" if index % 2 else "Medium",
                    "enterprise_fit_label": "Team Ready" if pricing in {"Enterprise", "Custom Quote"} else "Growing Teams",
                    "short_verdict": f"Strong fit when you need {task.lower()} with a curated {capability.lower()} workflow.",
                    "pros": [
                        "Clear use case positioning",
                        "Structured output format",
                        f"{integration} entry point available",
                    ],
                    "cons": [
                        "Sample data only in local scaffold",
                        "No in-platform execution in Phase 1",
                    ],
                },
                "use_cases": [
                    {"title": f"{industry} workflow review", "description": f"Review recurring {industry.lower()} work with an agent-first workflow."},
                    {"title": f"{task} automation", "description": f"Accelerate {task.lower()} for small teams."},
                ],
                "integrations": [
                    {"name": integration, "type": integration.lower(), "notes": f"Supports {integration} entry flows."},
                ],
            }
        )
    ...
    content = [
        {
            "slug": "best-agents-for-finance-teams",
            "title": "Best Agents for Finance Teams",
            "summary": "How to compare curated finance agents by workflow fit and setup effort.",
            "post_type": "ranking",
            "content_md": "# Best Agents for Finance Teams\n\nUse the catalog to compare finance-focused agents by task, integration, and enterprise fit.",
        },
        {
            "slug": "best-agents-for-research",
            "title": "Best Agents for Research",
            "summary": "A quick guide to evaluating research agents by clarity, setup, and output quality.",
            "post_type": "ranking",
            "content_md": "# Best Agents for Research\n\nResearch-heavy teams should prioritize clarity, evidence, and output structure.",
        },
        {
            "slug": "how-to-choose-the-right-agent",
            "title": "How to Choose the Right Agent",
            "summary": "A Phase 1 selection guide for teams evaluating curated agents.",
            "post_type": "guide",
            "content_md": "# How to Choose the Right Agent\n\nStart with task fit, then compare setup friction and integration surface.",
        },
        {
            "slug": "agent-vs-workflow-tool",
            "title": "Agent vs Workflow Tool",
            "summary": "When to choose an agent, when to choose a workflow tool, and when you need both.",
            "post_type": "comparison",
            "content_md": "# Agent vs Workflow Tool\n\nThe right answer depends on autonomy, repeatability, and oversight needs.",
        },
    ]

    (data_dir / "seed_taxonomy.json").write_text(json.dumps(taxonomy, ensure_ascii=False, indent=2), encoding="utf-8")
    (data_dir / "seed_agents.json").write_text(json.dumps(agents, ensure_ascii=False, indent=2), encoding="utf-8")
    (data_dir / "seed_content.json").write_text(json.dumps(content, ensure_ascii=False, indent=2), encoding="utf-8")
    script_path.write_text(
        "\n".join(
            [
                ...
                "DATA_DIR = ROOT / \"apps\" / \"api\" / \"src\" / \"data\"",
                "",
                "",
                ...
                "def main() -> int:",
                "    DATA_DIR.mkdir(parents=True, exist_ok=True)",
                "    (DATA_DIR / \"seed_taxonomy.json\").write_text(json.dumps(build_taxonomy(), ensure_ascii=False, indent=2), encoding=\"utf-8\")",
                "    (DATA_DIR / \"seed_agents.json\").write_text(json.dumps(build_agents(), ensure_ascii=False, indent=2), encoding=\"utf-8\")",
                "    (DATA_DIR / \"seed_content.json\").write_text(json.dumps(build_content(), ensure_ascii=False, indent=2), encoding=\"utf-8\")",
                "    print(\"Seed files written to apps/api/src/data\")",
                "    return 0",
                "",
                "",
    ...
    return [
        str(script_path),
        str(data_dir / "seed_agents.json"),
        str(data_dir / "seed_content.json"),
        str(data_dir / "seed_taxonomy.json"),
    ]
...
def _route_name_to_agent_role(route_name: str) -> AgentRole:
    mapping = {
        ...
        "runtime_qa": AgentRole.RUNTIME_QA,
...
def _build_fix_fingerprint(repo_b_path: Path, candidate_files: list[str]) -> str:
    digest = hashlib.sha1()
    for relative_path in sorted({str(item).strip() for item in candidate_files if str(item).strip()}):
        target = repo_b_path / relative_path
